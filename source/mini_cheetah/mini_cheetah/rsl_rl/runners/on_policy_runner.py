...
import rsl_rl
from mini_cheetah.rsl_rl.algorithms import PPO, Distillation, KoopmanPPO
from mini_cheetah.rsl_rl.env import VecEnv
from mini_cheetah.rsl_rl.modules import (
    ActorCritic,
...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        policy: ActorCritic | ActorCriticRecurrent | StudentTeacher | StudentTeacherRecurrent | LatentStateActorCritic = policy_class(
        ...
        self.save_interval = 50 #self.cfg["save_interval"]
        # input(self.save_interval)
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
        ...
        if self.log_dir is not None and self.writer is None and not self.disable_logs:
            ...
            if self.logger_type == "neptune":
                from mini_cheetah.rsl_rl.utils.neptune_utils import NeptuneSummaryWriter

                self.writer = NeptuneSummaryWriter(log_dir=self.log_dir, flush_secs=10, cfg=self.cfg)
                self.writer.log_config(self.env.cfg, self.cfg, self.alg_cfg, self.policy_cfg)
            elif self.logger_type == "wandb":
                from mini_cheetah.rsl_rl.utils.wandb_utils import WandbSummaryWriter
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    # At this point, `obs` and `privileged_obs` are for the current time t.
                    # `actions` are a_t.
                    current_critic_obs = privileged_obs
                    ...
                    next_critic_obs = privileged_obs

                    next_value = self.alg.policy.evaluate_next_value(current_critic_obs, actions, next_critic_obs).detach()
                ...
                # compute returns
                if self.training_type == "rl":
                    if isinstance(self.alg, KoopmanPPO):
                        self.alg.compute_returns(privileged_obs, actions)
                    else:
