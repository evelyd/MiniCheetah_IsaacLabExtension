...
- Throughout the repository, the name `mini_cheetah` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of mini_cheetah (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e source/mini_cheetah
```
...
We provide an example UI extension that will load upon enabling your extension defined in `source/mini_cheetah/mini_cheetah/ui_extension_example.py`.
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/source/mini_cheetah"
    ]
}
```
