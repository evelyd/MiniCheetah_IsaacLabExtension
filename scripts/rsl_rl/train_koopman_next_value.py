...
# no base pose in obs, controlled DAE 05-16_16-16-41, trained with state ['joint_pos_S1', 'joint_vel', 'base_vel', 'base_ang_vel', 'projected_gravity', 'a_joint_pos_S1'], action ['velocity_commands_xy', 'velocity_commands_z']
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    print(f"[INFO] Using seed: {agent_cfg.seed} for environment.")
    ...
    # create runner from rsl-rl that uses the KoopmanPPO
    # input(f"agent cfg after: {agent_cfg.to_dict()}")
    alg_cfg = agent_cfg.to_dict()["algorithm"]
    alg_cfg.update({
        "class_name": "KoopmanPPO",
    })
    agent_cfg.algorithm = alg_cfg

    # use LatentStateActorCritic
